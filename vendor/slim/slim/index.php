...
$app->get('/desplegarBandas', function(){
    $mysql=iniciarSesion();
    $query="select * from banda where completa=1";
    $results = $mysql->query($query);

    while ($row = mysqli_fetch_array($results, MYSQLI_BOTH)) 
    {
      $idBanda=$row[0];
      $resultGenero= $mysql->query("select nombre from genero where idgenero=".$row[8]);
      $Genero = mysqli_fetch_array($resultGenero, MYSQLI_BOTH);

       echo '<tr>';
       echo '<td class="col-xs-3 col-md-4"><div class="album"><a href="#" class="listen" data-toggle="modal" data-target="#albumdetail" onclick="ElegirBanda('.$idBanda.')"><img src="'.$row[4].'" class="img-responsive img-rounded" alt="music theme" /><div class="albumdetail"><h5>'.$row[1].'</h5><span class="glyphicon glyphicon-headphones"></span> Más información </a></div></div></td>';
       echo '<td class="col-xs-3 col-md-4"><br><h1 style="font-size: 300%;"><span class="label label-danger">'.$Genero[0].'</span></h1></div></td>';
       echo '<td class="col-xs-3 col-md-4"></br><h1 style="font-size: 300%;" class="pricing-rate"><sup class="text-muted">$</sup>'.$row[5].'<small>/MX</small></h1><span class="label label-success">Precio por hora</span></div></div></td>';
       echo '</tr>';
     }
